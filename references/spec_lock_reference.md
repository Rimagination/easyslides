...
> Optional; include only when a deck intentionally binds to a reusable style pack rather than a copied project template. This section is a machine-readable style contract: Executor must use the named package's tokens, allowed themes, and layout registry as the authority for all pages that reference `page_layouts`.
>
> Current supported package: `guizang_ppt`.
>
> Valid Guizang variants/themes:
> - `style_a_editorial_ink`: `monocle_classic`, `indigo_porcelain`, `forest_ink`, `kraft_paper`, `dune`
> - `style_b_swiss`: `ikb`, `lemon`, `lemon_green`, `safety_orange`
>
> When `style_pack` is present, `page_layouts` values may use style-pack layout IDs or package-relative SVG basenames, for example `S08`, `swiss/S08_duo_compare`, or `templates/style_packs/guizang_ppt/swiss/S08_duo_compare.svg`. Validate before execution with:
>
> ```
...
> `library` MUST be exactly one of `lucide` / `chunk-filled` / `tabler-filled` / `tabler-outline` / `phosphor-duotone` — mixing is forbidden. Prefer `lucide` for new generic icons and emoji replacement unless the template locks a different family. `brand_library: simple-icons` is optional; include only when the deck uses real company / product brand marks, otherwise omit. `inventory` lists approved icon names (no library prefix); Executor may only use icons from this list.
>
> **`stroke_width` (stroke-style libraries only)** — required when `library` is stroke-based (currently `lucide` / `tabler-outline`); allowed values `1.5` / `2` / `2.25` / `3`. Executor MUST apply this value to every `<use data-icon="...">` placeholder via `stroke-width`, deck-wide. Omit for non-stroke libraries (`chunk-filled` / `tabler-filled` / `phosphor-duotone`) — ignored there. For heavier weight switch library; do not exceed `3` (at 24×24 strokes merge and the icon stops reading as line art).
>
...
> One entry per page **that uses a template SVG**. Key: `P<NN>` matching §IX. Value: the template's SVG basename without extension (e.g., `01_cover`, `03a_content_image_text`) — Executor resolves it to `templates/layouts/<chosen_template>/<value>.svg`. Modern templates ship many content-page variants (`03a_content_abstract`, `03b_content_image_text`, `03c_content_three_items` …); the page-type → single-file mapping in `executor-base.md §1` no longer covers them, so this section is the per-page truth.
>
> **Style-pack override**: when `## style_pack` is present, values are resolved through the package registry first. For Guizang, `P02: S08` and `P02: swiss/S08_duo_compare` both resolve to `templates/style_packs/guizang_ppt/swiss/S08_duo_compare.svg`. This lets a deck inherit a Guizang visual system without copying those SVGs into a project-local `templates/layouts/<chosen_template>/` folder.
>
...
>
> **Strategist source**: copy the per-page SVG choices from `design_spec.md §VI Page Roster` (or §IX outline if Roster is absent). Names must match files in `templates/layouts/<chosen_template>/` exactly — typos cause silent fallback to free design.
